...
**How to apply:** Config = `env.openai.{apiKey, baseUrl, chatModel, embeddingModel}`. `OPENAI_BASE_URL` (empty = api.openai.com, normalized via `normalizeEndpoint`) points at any OpenAI-compatible provider WITHOUT code changes — add this first if switching providers. `getOpenAi()` takes NO args (single cached client). `isOpenAiUsable() = enabled && apiKey`; `isChatUsable`/`isEmbeddingsUsable` alias it; `isVisionUsable` also needs `imageAnalysisEnabled`. If AI "doesn't work" on plain OpenAI, suspect ENV DELIVERY (OPENAI_ENABLED/OPENAI_API_KEY not reaching the container — see Coolify pass-through note) or `OPENAI_ENABLED` defaulting false, NOT model names.
